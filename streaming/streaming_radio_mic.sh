...
INPUT_RADIO="http://relay.publicdomainradio.org:80/jazz_swing.mp3"
...
# Set Audio mic input (check using "arecord -l")
...
raspivid -o - -t 0 -w $WIDTH -h $HEIGHT -fps $FRAMERATE -b $BITRATE -rot $ROTATION -g $KEYFRAME -f | ffmpeg -use_wallclock_as_timestamps 1 -thread_queue_size 10240 -f h264 -r 30 -i - -itsoffset $OFFSET -thread_queue_size 10240 -ac 2 -i $INPUT_RADIO -f alsa -i $INPUT_MIC -vcodec copy -acodec copy -ac 2 -ar 44100 -ab 192k -f flv "${URL}/${KEY}"