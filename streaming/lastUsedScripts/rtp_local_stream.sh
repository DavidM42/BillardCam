...
# TODO check if helps reduce motion blur
# exposure time (see https://raspberrypi.stackexchange.com/a/63087)
EXPOSURE=antishake
# other option which could help is sports

URL=rtsp://192.168.178.1:8554/example
...
raspivid -n -o - -t 0 -w $WIDTH -h $HEIGHT -fps $FRAMERATE -b $BITRATE -g $KEYFRAME -ex $EXPOSURE | ffmpeg -use_wallclock_as_timestamps 1 \
    -nostdin \
